...
.bg-all-present { background-color: #d4edda; }
.bg-partial { background-color: #fff3cd; }
.bg-none { background-color: #f8d7da; }
</style>
</head>
...
<h3 class="mb-4">
Monthly Attendance - <?= date('F Y', mktime(0,0,0,$month,1)) ?>
</h3>
...
for($day=1;$day<=$daysInMonth;$day++) {
    ...
    if($presentCount == count($employees) && count($employees) > 0)
        $dayClass .= ' bg-all-present';
    ...
    foreach($employees as $emp){
        ...
        if($fullAttendance)
            echo "<i class='fas fa-trophy full-attendance' title='Full Attendance'></i>";
