...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (patientForm) {
    patientForm.addEventListener('submit', (e) => {
      ...
      const phoneRegex = /^0?\d{10}$/; // optional leading 0, 10 digits after

      if (dataObj.cnic && !cnicRegex.test(dataObj.cnic)) {
        notyf.error('CNIC must be exactly 13 digits (no "-").'); return;
      }
      if (dataObj.mobile_no && !phoneRegex.test(dataObj.mobile_no)) {
        notyf.error('Mobile number must be 10 or 11 digits (start with 0 optional).'); return;
      }
      if (dataObj.guardianphonenumber && !phoneRegex.test(dataObj.guardianphonenumber)) {
        notyf.error('Guardian phone must be 10 or 11 digits (start with 0 optional).'); return;
      }
